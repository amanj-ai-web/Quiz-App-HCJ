...
function loadNavbar(selector = "#navbar") {
  fetch("../components/navbar.html")
    ...
    .then((data) => {
      ...
      if (globalThis.location.href.includes("quiz.html")) checkQuizStatus();
      else if (globalThis.location.href.includes("index.html"))
...
function loadQuizReview() {
  ...
  document.getElementById("answered").textContent =
    `${currentUser.score}/${currentUser.questions.length}`;
...
function setNavbar() {
  ...
  if (!navItems) {
    console.warn("nav-items not found yet");
    return;
  }
...
function submitQuiz() {
  ...
  localStorage.setItem("users", JSON.stringify(users));
  localStorage.setItem("currentUser", JSON.stringify(currentUser));
  globalThis.location.href = "review.html";
}
...
if (globalThis.location.href.includes("quiz.html"))
